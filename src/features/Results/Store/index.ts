...
export function useFormattedResults() {
  const [formattedResultsDocument, setFormattedResultsDocument] = useState<
    any[]
  >();
  let [markedResultsForDiscussion, setMarkedResultsForDiscussion] = useState(
    new Set()
  );

  useEffect(() => {
    console.log(markedResultsForDiscussion);
  }, [markedResultsForDiscussion.size]);
  ...
  return {
    ...
    setFormattedResultsDocument,
    markedResultsForDiscussion,
    setMarkedResultsForDiscussion
  };
}
